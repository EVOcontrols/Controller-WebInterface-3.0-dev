...
export const useFuncsStore = defineStore('funcsStore', () => {
    ...
    const funcLabels = ref<{ name: 'udf-act' | 'udf-cond' | 'udf-trans' | 'udf-trig'; val: string[] }[][]>([]);

    function setFuncsNumberPerPage(value: FuncsNumberPerPage) {
        funcsNumberPerPage.value = value;
    }

    function setLabels(device: number, type: 'udf-act' | 'udf-cond' | 'udf-trans' | 'udf-trig', labelsArr: string[]) {
